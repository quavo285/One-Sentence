(function (m) {
  ...
  MtaH5.hack = function () {
    ...
    if (0 == u.length)
      for (
        var w = document.getElementsByTagName("script"), v = 0;
        v < w.length;
        v++
      )
        if (
          "undefined" !== typeof w[v].attributes.name &&
          "MTAH5" == w[v].attributes.name.nodeValue
        ) {
          u = [];
          u.push(w[v]);
          break;
    ...
    0 < u.length &&
      (function () {
        "undefined" !== typeof u[0].attributes.sid &&
          (r.conf.sid = u[0].attributes.sid.nodeValue);
        "undefined" !== typeof u[0].attributes.cid &&
          (r.conf.cid = u[0].attributes.cid.nodeValue);
        "object" === typeof _mtac &&
          (function () {
            for (var n in _mtac)
              if ("ignoreParams" == n) {
                ...
                "object" == typeof _mtac[n] &&
                  0 < _mtac[n].length &&
                  _mtac.hasOwnProperty(n) &&
                  (r.conf[n] = _mtac[n]);
...
(function (m, u) {
  ...
  function w(a, c, e) {
    if (window.localStorage)
      ...
      var b = window.location.host,
        ...
        k = b.split(".");
      2 < k.length &&
        (b = (t[k.slice(-2).join(".")] ? k.slice(-3) : k.slice(-2)).join("."));
      document.cookie =
  ...
  function x() {
    ...
    c.pvi = (function () {
      ...
      if (m.userReport) {
        var b = r("pgv_uid");
        (b && b == m.user.user_id) ||
          ((c.ty = 1), w("pgv_uid", m.user.user_id, e));
        b = m.user.user_id;
      } else
        (b = r("pgv_pvi")),
          b || ((c.ty = 1), (b = n(void 0)), w("pgv_pvi", b, e));
      return b;
    })();
    c.si = (function () {
      var e = r("pgv_si");
  ...
  function z(a) {
    ...
    if (a) {
      ...
      for (b in a)
        a.hasOwnProperty(b) &&
          e.push(encodeURIComponent(b) + "=" + encodeURIComponent(a[b]));
      a = e.join(";");
      c.ext = a;
    }
    return arr;
  ...
  function B(a) {
    ...
    if (m.sid)
      if (
        !m.userReport ||
        (m.user.user_id && !parseInt(m.user.user_id, 10) !== m.user.user_id)
      ) {
        a = [];
        for (
          var e = 0,
          ...
          for (c in b[e])
            b[e].hasOwnProperty(c) &&
              a.push(c + "=" + ("undefined" == typeof b[e][c] ? "" : b[e][c]));
        var k = function (d) {
          d = Ta.src =
            "https://pingtas.qq.com/webview/pingd?" + d.join("&").toLowerCase();
          var f = new Image();
          Ta[m.sid] = f;
          ...
          ((c = function () {
            if (window.performance) {
              ...
              var l = { value: d.connectEnd - d.connectStart },
                h = {
                  value:
                    d.responseStart - (d.requestStart || d.responseStart + 1),
                },
              ...
              d = {
                ...
                domParsingTime: {
                  value: d.domContentLoadedEventStart
                    ? d.domInteractive - d.domLoading
                    : -1,
                },
                domContentLoadedTime: {
                  value: d.domContentLoadedEventStart
                    ? d.domContentLoadedEventStart - d.fetchStart
                    : -1,
                },
              };
            } else d = "";
            ...
            for (var p = g.length; h < p; h++)
              for (var q in g[h])
                g[h].hasOwnProperty(q) &&
                  d.push(
                    q + "=" + ("undefined" == typeof g[h][q] ? "" : g[h][q])
                  );
            for (q in f)
              f.hasOwnProperty(q) &&
                ("domContentLoadedTime" == q
                  ? d.push("r3=" + f[q].value)
                  : l.push(f[q].value));
      ...
      for (d = k.length; f < d; f++)
        for (var l in k[f])
          k[f].hasOwnProperty(l) &&
            t.push(l + "=" + ("undefined" == typeof k[f][l] ? "" : k[f][l]));
  ...
  Ta.clickShare = MtaH5.clickShare = function (a) {
    ...
    if (e.cid) {
      ...
      for (d = k.length; a < d; a++)
        for (var f in k[a])
          k[a].hasOwnProperty(f) &&
            b.push(f + "=" + ("undefined" == typeof k[a][f] ? "" : k[a][f]));
      f = MtaH5.src =
        "https://pingtas.qq.com/webview/pingd?" + b.join("&").toLowerCase();
      var l = new Image();
